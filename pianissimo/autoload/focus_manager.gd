...
const BASE_REWARD: int = 20
const REWARD_PER_MINUTE: int = 4
...
func _check_for_crashed_session() -> void:
	# If this flag is still true = app was killed mid-session, not just backgrounded
...
func _on_tick() -> void:
	...
	var reward: int = BASE_REWARD + int((target_seconds/60.0) * REWARD_PER_MINUTE)
	Economy.add(reward)
	last_completed = true
	last_reward = reward
